...
public final class GsalesDao
{
	...
	public ArrayList<Gsales> dailyGsales()
	{
		...
		String sql = "select gname,gprice,gnum, allSum from goods, (select gid as salesid,sum(snum) as allSum from gsales where current_timestamp = sdate group by gid) as totle  where gid = salesid";
		try
		{
	...
	public boolean shoppingSettlement(Gsales gSales)
	{
		boolean bool = false;
		conn = DbConn.getconn();
		String sql = "INSERT INTO GSALES(GID,SID,SNUM,SDATE) VALUES(?,?,?,now())";
